	...
	public class DeviceInfoCounterData: BaseCounterData
	{
		...
		internal override void UpdateValue(bool force)
		{
			...
			if (screenData)
			{
				...
				text.Append("SCR: ").Append(res.width).Append("x").Append(res.height).Append("@").Append(res.refreshRateRatio).Append("Hz [window size: ").Append(Screen.width).Append("x").Append(Screen.height);
